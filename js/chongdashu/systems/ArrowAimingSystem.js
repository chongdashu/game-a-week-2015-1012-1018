...
/**
 * ArrowAimingSystem
 * @class ArrowAimingSystem
 * @constructor
 **/
var ArrowAimingSystem = function(state) {
    this.init(state);
};
var p = chongdashu.Utils.extend(ArrowAimingSystem, chongdashu.System);
    ...
    {
        console.log("[ArrowAimingSystem], init()");
    ...
    p.update = function() {
        ...
        if (this.kc) {
            if (this.kc.isJustDown(Phaser.Keyboard.SPACEBAR)) {
                ...
                var offsetY = -8 + (this.state.player.anchor.y == 1 ? -32 : 0 );
                var arrow = this.arrowGroup.create(this.state.player.x+offsetX, this.state.player.y+offsetY, "arrow");
...
// Link
// ----
chongdashu.ArrowAimingSystem = chongdashu.Utils.promote(ArrowAimingSystem, "System");
